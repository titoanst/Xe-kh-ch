    ...
    >
      <div className="container mx-auto px-4 md:px-6 flex items-center justify-between">
        ...
        <a href="#" className="flex items-center gap-2 group">
          ...
          <span className={`font-bold text-xl md:text-2xl ${isScrolled ? 'text-primary-800' : 'text-primary-900 drop-shadow-sm'}`}>
            {APP_NAME}
          </span>
        </a>
        ...
        <nav className="hidden md:flex items-center gap-8">
          ...
          <a 
            ...
            className="bg-brand-orange hover:bg-orange-600 text-white font-bold py-2 px-6 rounded-full shadow-lg transform hover:scale-105 transition-all animate-pulse"
          >
            Gọi {HOTLINE}
          </a>
        </nav>
        ...
        {/* Mobile Menu Button */}
        <button 
          className="md:hidden p-2 text-primary-800 bg-white/90 rounded-lg backdrop-blur-sm"
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-white shadow-xl border-t border-gray-100">
          <div className="flex flex-col p-4 gap-4">
            ...
            <a 
              href={`tel:${HOTLINE.replace(/\./g, '')}`} 
              className="bg-brand-orange text-white font-bold py-3 rounded-lg text-center shadow-md"
            >
              Gọi ngay {HOTLINE}
            </a>
          </div>
